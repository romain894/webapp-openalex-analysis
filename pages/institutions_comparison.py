...
def update_table(active_cell, data, element_type_counted, concept):
    ...
    if active_cell:
        if element_type_counted == 'reference':
            ...
            # if the selected column is an institution
            if active_cell['column_id'][0] == "I":
                institution = active_cell['column_id']
                institution = institution[0:institution.find(" ")]
                # https://openalex.org/ --> len(21)
                work = data[active_cell['row']]['Element link'][21:]
                div_children.append(html.Br())
                div_children.append(                    
                    html.A("Works and Authors citing this work",
                        href=dash_app_config.config_app_url+"citations-analysis?concept="+concept+"&institution="+institution+"&reference="+work,
                        target="_blank"
                    ),
                )
        else:
